...
void file_transmit(int sock, FILE *file_name) {
    ...
    send_to_socket(temp_buff, sock, 1, "exit"); // send transmission complete message
    read_from_socket(temp_buff, sock); // receive confirmation
}
...
void get_return_message(int sock, char *content) {
    ...
    while (loop) {
        ...
        send_to_socket(temp_buffer, sock, 1, "ack");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    get_return_message(new_sock, decrypted);
